class FDTree(object):
    def __init__(self, U):
        self.U = U
        self.root = [[None]*len(U), [False]*len(U)]
        self.n_fds = 0
    ...
    def add_fd(self, ant, con):
        ...
        while bool(ant):
            ...
            if current_node[0][x] is None:
                current_node[0][x] = [[None]*len(self.U), [False]*len(self.U)]
    ...
    def read_and_iterate(self, current_node, base):
        ...
        for i, j in filter(lambda (i,j): type(j) == list, enumerate(current_node[0])):
    ...
    def l_close_and_iterate(self, current_node, ant):
        for i in range(len(self.U)):
            if current_node[1][i] and i not in ant:
                yield i
            if current_node[0][i] is not None and i in ant:
